...
class Verification::Validator::SourcesInCorrectQuadrants < ActiveModel::Validator
  def validate(record) # rubocop:todo Metrics/MethodLength
    ...
    if destination_plate.nil?
      record.errors.add(:base, 'Couldn\'t find the destination plate.')
      return
    end
    ...
    if missing_custom_metadatum_collection?(destination_plate)
      record.errors.add(:base, 'The destination plate doesn\'t have any quadrant information. ' \
  ...
  def validate_quadrants(record, destination_plate) # rubocop:todo Metrics/MethodLength
    (1..4).each do |index|
      ...
      if quad_metadata.nil?
        record.errors.add(:base, "The destination plate doesn't have any information for quadrant #{index}")
        break
      end
      ...
      record.errors.add(:base, "The barcode in bed #{bed} doesn't match the plate in " \
