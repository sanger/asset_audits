    ...
    [%w[123 abc], ['', nil], ['abc', nil]].each do |witness_barcode, witness_login|
      context "where invalid witness barcodes are scanned with #{witness_barcode}" do
        setup do
          ...
          User.expects(:login_from_user_code).at_least(0).with(@input_params[:witness_barcode]).returns(witness_login)
        ...
        should 'return an error' do
          assert_includes @bed_layout_verification.errors[:witness_barcode], 'Invalid witness barcode'
        end
