class AddBiorobot < ActiveRecord::Migration
  ...
  def self.up
    ActiveRecord::Base.transaction do
      instrument = Instrument.create! ({:name => "Biorobot", :barcode => self.ROBOT_BARCODE })
      ...
      instrument_processes_instrument = InstrumentProcessesInstrument.create! ({:instrument => instrument,
        ...
        self.BEDS.each_with_index.map do |b, pos|
  ...
  def self.down
    ActiveRecord::Base.transaction do
      self.BEDS.each{|b| Bed.find_by_barcode(b).destroy }
