...
class LabwareApiTest < ActiveSupport::TestCase
  ...
  setup do
    ...
    @barcodes = ['labware-1-barcode', 'labware-2-barcode']
    ...
    context '#location_info' do
      context 'when request is succesful' do
        setup do
          @location_info = {
            depth: 2,
            labwares: [
              { barcode: 'labware-1-barcode' },
              { barcode: 'labware-2-barcode' }
            ]
          }
          stub_request(:get, (@location_uri.to_s + "?barcode=#{@location_barcode}"))
                    .to_return(body: @location_info.to_json, status: 200)
      ...
      context "when an unknown error occurs on the LabWhere side" do
        setup do
          ...
          stub_request(:get, (@location_uri.to_s + "?barcode=#{@location_barcode}"))
                    .to_return(body: {}.to_json, status: 500) # Use a different status code
          @result = @dummy.location_info(@location_barcode)
        end
