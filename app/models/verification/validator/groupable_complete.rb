...
class Verification::Validator::GroupableComplete < ActiveModel::Validator
  ...
  def is_transfer_valid?(transfer, record) # rubocop:disable Naming/PredicatePrefix
  ...
  def is_transfer_complete?(transfer, scanned_values) # rubocop:disable Naming/PredicatePrefix
    transfer_bed_names(transfer).all? do |bed|
      bed_barcode(scanned_values, bed).present? && plate_barcode(scanned_values, bed).present?
    end
  end

  def is_transfer_empty?(transfer, scanned_values) # rubocop:disable Naming/PredicatePrefix
    transfer_bed_names(transfer).all? do |bed|
