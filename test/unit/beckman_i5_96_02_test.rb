...
class BeckmanI59602Test < ActiveSupport::TestCase
  setup do
    ...
    config_folders = %w[instruments instrument_processes instrument_processes_instruments beds]
    config_folders.each do |folder|
      # Find the directory and files to load.
      directory = Rails.root.join("config/default_records/#{folder}")
      files =
        Dir
          .glob(directory.join("*beckman_i5_96_02*"))
          .map do |file|
    ...
    @child_plate = FactoryBot.create(:v2_plate_with_parent, barcode: "child-barcode", parent_barcode: "parent-barcode")

    Sequencescape::Api::V2::Plate.stubs(:where).with(barcode: "parent-barcode").returns([@parent_plate])
    Sequencescape::Api::V2::Plate.stubs(:where).with(barcode: "child-barcode").returns([@child_plate])
  end
  ...
  def create_input_params(source_bed_no, source_plate, dest_bed_no, dest_plate)
    {
      ...
      robot: {
        p2: {
          bed: @beds[source_bed_no].barcode,
          plate: source_plate
        },
        p3: {
          bed: "",
  ...
  context "when valid barcodes are scanned" do
    setup do
      ...
      @bed_layout_verification =
        Verification::DilutionPlate::I5.new(
